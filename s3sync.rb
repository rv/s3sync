...
ENDUSAGE
		...
		$S3SyncInclude = Regexp.new($S3syncOptions['--include']) if $S3syncOptions['--include']
		...
		localTree = Generator.new do |g|
            ...
            if $S3SyncInclude.nil? or $S3SyncInclude.match(excludePath) # zzz
              # a normal looking node we should try to process
						  debug("local item #{fullPath}")
						  g.yield(LocalNode.new(prefix, partialPath))
            else
              debug("Local item {excludePath} skipped because of --include")
            end
					end
				end
				#if $S3syncOptions['--memory']
